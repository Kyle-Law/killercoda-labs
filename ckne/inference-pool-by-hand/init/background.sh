...
# Everything the learner does NOT write: the picker itself, its identity, the
# permissions it needs, and the Istio-specific TLS rule. The InferencePool and
# the HTTPRoute are deliberately absent -- writing those is step 1.
...
spec:
  replicas: 1
  selector:
    matchLabels: {app: llm-epp}
  template:
    metadata:
      labels: {app: llm-epp}
    spec:
      serviceAccountName: llm-epp
      containers:
      - name: epp
        image: registry.k8s.io/gateway-api-inference-extension/epp:v1.5.0
        args: ["--pool-name", "llm-pool", "--pool-namespace", "default",
               "--pool-group", "inference.networking.k8s.io", "--v", "1"]
        ports:
...
metadata:
  name: llm-epp
spec:
  selector: {app: llm-epp}
  ports:
  - {name: grpc-ext-proc, port: 9002, protocol: TCP}
---
# The picker serves ext_proc over TLS. Istio has to be told, and there is no
# Gateway API field that says so.
...
spec:
  ...
  - name: http
    ...
    protocol: HTTP
EOF
kubectl rollout status deploy llm-epp --timeout=300s >/dev/null 2>&1

# Kept on disk so step 3 can be undone without retyping it.
cat > /root/destinationrule.yaml <<'EOF'
apiVersion: networking.istio.io/v1beta1
kind: DestinationRule
metadata:
  name: llm-epp
...
kubectl get crd inferencepools.inference.networking.k8s.io >/dev/null 2>&1 || MISSING="$MISSING CRD/inferencepools"
kubectl get deploy vllm-qwen3-32b >/dev/null 2>&1 || MISSING="$MISSING Deployment/vllm-qwen3-32b"
kubectl get deploy llm-epp >/dev/null 2>&1 || MISSING="$MISSING Deployment/llm-epp"
[ -n "$MISSING" ] && echo "$MISSING" > /tmp/.initbroken


# One request through the gateway, reporting the status code and how long it
# took -- the timing matters, because a failure that returns in under a
# millisecond did not reach a model server.
cat > /usr/local/bin/poolcall <<'WRAP'
#!/bin/bash
PORT=$(kubectl get svc -l gateway.networking.k8s.io/gateway-name=inference-gateway \
        -o jsonpath='{.items[0].spec.ports[?(@.port==80)].nodePort}' 2>/dev/null)
[ -z "$PORT" ] && { echo "No NodePort for the Gateway yet."; exit 1; }
BODY=$(mktemp)
CODE=$(curl -s -m 30 -o "$BODY" -w '%{http_code} %{time_total}' \
  "http://localhost:$PORT/v1/completions" -H 'Content-Type: application/json' \
  -d "{\"model\":\"Qwen/Qwen3-32B\",\"prompt\":\"${1:-hello}\",\"max_tokens\":20}")
echo "  http=$(echo $CODE | cut -d' ' -f1)   time=$(echo $CODE | cut -d' ' -f2)s"
echo "  body: $(head -c 200 "$BODY")"
rm -f "$BODY"
WRAP
chmod +x /usr/local/bin/poolcall

# What the picker itself says. Almost every failure in this lab is legible here
# and nowhere else.
cat > /usr/local/bin/pickerlog <<'WRAP'
#!/bin/bash
echo "--- picker pods:"
kubectl get pods -l app=llm-epp --no-headers 2>/dev/null | sed 's/^/  /'
echo "--- errors (most recent last):"
kubectl logs -l app=llm-epp --tail="${1:-40}" 2>/dev/null \
  | grep -oE '"error":"[^"]{0,200}' | sed 's/^/  /' | tail -5
echo "--- if that is empty, the picker has no complaints of its own."
WRAP
chmod +x /usr/local/bin/pickerlog
