...
> **What production actually does.** This gap is exactly why the [Gateway API Inference Extension](https://gateway-api-inference-extension.sigs.k8s.io/) exists. It adds an `InferencePool` — a set of model-server replicas fronted by an *endpoint picker*, an external process the gateway consults per request, which scrapes each replica's `vllm:num_requests_waiting` and returns the least-loaded one. A companion component, the body-based router, reads the model name out of the JSON body, so routing no longer needs the header you just had to invent.
>
> Body-aware routing and queue-aware balancing are the same underlying problem — both need to *understand* the request rather than merely forward it. They are not, however, the same component: as of GAIE v1.6 the endpoint picker lives in `llm-d/llm-d-router` and the body-based router in `llm-d/llm-d-inference-payload-processor`, with only the `InferencePool` API and the picker protocol remaining in the extension itself.
