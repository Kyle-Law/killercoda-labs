...
REPO=$(kubectl get application solar-system -n argocd -o jsonpath='{.spec.source.repoURL}' 2>/dev/null)
echo "$REPO" | grep -q "gitea.gitea.svc.cluster.local" || exit 1
...
for i in $(seq 1 30); do
  ...
  if [ "$SYNC" == "Synced" ] && [ "$HEALTH" == "Healthy" ] && [ "$READY" == "2" ] && [ "$SVC_TYPE" == "NodePort" ]; then
