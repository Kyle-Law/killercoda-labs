...
`kiamol/day10-helm` covers the happy path: install, override values, roll back, package a chart. This lab covers what happens when an upgrade goes wrong — which is most of what actually matters under exam pressure, because a release's STATUS is not the same claim as "the app is running".
...
- `podinfo3` — `--rollback-on-failure` goes one step further: on failure, Helm rolls the release back for you, automatically. (You'll see this called `--atomic` in older material — same thing, deprecated spelling.)
